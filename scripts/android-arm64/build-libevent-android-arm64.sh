#!/usr/bin/env bash
# build-libevent-android-arm64.sh
# Builds libevent (https://github.com/libevent/libevent) for Android arm64-v8a
# Shared library only, no tests, no samples.
set -euo pipefail
...
# ------------------------------------------------------------------
# 2.  libevent submodule / clone
# ------------------------------------------------------------------
LIBRARY_NAME="libevent"
REPO_DIR=".extensions/$LIBRARY_NAME"
[[ -d "$REPO_DIR" ]] || git clone --depth 1 --branch release-2.1.12-stable \
  https://github.com/libevent/libevent.git "$REPO_DIR"
cd "$REPO_DIR"
...
rm -rf build          # clean stale caches
