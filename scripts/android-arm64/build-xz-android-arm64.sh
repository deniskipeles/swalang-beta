#!/usr/bin/env bash
# build-xz-android-arm64.sh
# Builds XZ Utils / liblzma (https://github.com/tukaani-project/xz) for Android arm64-v8a
# Shared library only, no CLI tools, no tests.
set -euo pipefail
...
# ------------------------------------------------------------------
# 2.  XZ submodule / clone
# ------------------------------------------------------------------
LIBRARY_NAME="xz"
REPO_DIR=".extensions/$LIBRARY_NAME"
[[ -d "$REPO_DIR" ]] || git clone --depth 1 --branch v5.6.2 \
  https://github.com/tukaani-project/xz.git "$REPO_DIR"
cd "$REPO_DIR"
