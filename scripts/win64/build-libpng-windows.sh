#!/usr/bin/env bash
# build-libpng-windows.sh  –  cross-compile libpng for Windows x64 (MinGW-w64)
set -euo pipefail
...
echo "🧬 2. Clone official libpng"
LIBRARY_NAME="libpng"
REPO_DIR=".extensions/$LIBRARY_NAME"
[[ -d "$REPO_DIR" ]] || \
    git clone --depth=1 https://github.com/glennrp/libpng.git  "$REPO_DIR"
cd "$REPO_DIR"
...
cmake -B build \
  ...
  -DZLIB_ROOT=/usr/x86_64-w64-mingw32
