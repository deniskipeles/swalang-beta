#!/usr/bin/env bash
# build-libgit2-windows.sh  –  cross-compile libgit2 for Windows x64 (MinGW-w64)
set -euo pipefail
...
# Add the binutils package which provides 'dlltool'
sudo apt-get install -y build-essential git mingw-w64 ninja-build \
                        binutils-mingw-w64-x86-64

echo "🧬 2. Clone official libgit2"
LIBRARY_NAME="libgit2"
REPO_DIR=".extensions/$LIBRARY_NAME"
[[ -d "$REPO_DIR" ]] || \
    git clone --depth=1 https://github.com/libgit2/libgit2.git  "$REPO_DIR"
cd "$REPO_DIR"
