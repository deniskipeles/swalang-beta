#!/usr/bin/env bash
# build-libyaml-windows.sh  –  cross-compile libyaml for Windows x64 (MinGW-w64)
set -euo pipefail
...
echo "🧬 2. Clone official libyaml"
LIBRARY_NAME="libyaml"
REPO_DIR=".extensions/$LIBRARY_NAME"
[[ -d "$REPO_DIR" ]] || \
    git clone --depth=1 https://github.com/yaml/libyaml.git  "$REPO_DIR"
cd "$REPO_DIR"
