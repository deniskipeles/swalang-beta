#!/usr/bin/env bash
# build-libuv-windows.sh  –  cross-compile libuv for Windows x64 (MinGW-w64)
set -euo pipefail
...
echo "🧬 2. Clone official libuv source"
LIBRARY_NAME="libuv"
REPO_DIR=".extensions/$LIBRARY_NAME"
[[ -d "$REPO_DIR" ]] || git clone --depth=1 https://github.com/libuv/libuv.git  "$REPO_DIR"
cd "$REPO_DIR"

echo "🔨 3. CMake configure & build"
