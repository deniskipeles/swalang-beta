#!/usr/bin/env bash
# build-sqlite-linux.sh -- builds the SQLite3 amalgamation as a shared library.
set -euo pipefail
...
sudo apt-get install -y build-essential curl unzip tree
...
# ------------------------------------------------------------------
echo "🔨 3. Compile shared library"
rm -rf "${INSTALL_DIR}"
mkdir -p "${INSTALL_DIR}/lib" "${INSTALL_DIR}/include"

# Compile directly to the final destination directory
gcc -shared "${SQLITE_COMPILE_FLAGS[@]}" \
